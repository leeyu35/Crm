...
<hr>
  ...
  <div class="form-group">
    ...
    <div class="col-sm-2">
      <select  class="form-control" name="b_company" id="b_company">        
        <?php if(is_array($agentcompany)): $k = 0; $__LIST__ = $agentcompany;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$agentcompany): $mod = ($k % 2 );++$k;?><option value="<?php echo ($agentcompany["id"]); ?>" title="<?php echo ($agentcompany["title"]); ?>" <?php echo ($info[b_company]==$agentcompany[id]?'selected':''); ?> ><?php echo ($agentcompany["companyname"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
  ...
  <div class="form-group">
    ...
    <div class="col-sm-3">
        ...
        <input type="text" class="form-control" name="b_money" id="b_money" value="<?php echo ($info[b_money]); ?>">
   ...
   <label for="inputEmail3" class="col-sm-1 control-label">预计回款日期</label>
    <div class="col-sm-2">
    	<input type="text" name="b_time" class="Wdate form-control" id="b_time" onClick="WdatePicker()"value="<?php echo (date("Y-m-d",$info[b_time])); ?>">
    </div>
