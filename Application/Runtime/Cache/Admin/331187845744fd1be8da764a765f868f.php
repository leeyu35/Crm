...
<script language="javascript">
	$(document).ready(function(e) {
		...
		function bianhao(){
			...
			prid=$("#product_line1 option:selected").val();
			xhx="_";
			var mydate = new Date();
			 ...
			 }else
			 {
			  ...
			  str += mydate.getDate();
			 $.get("<?php echo U('Contract_num');?>",{advertiser:<?php echo ($info[advertiser]); ?>,on:"<?php echo ($info[contract_no]); ?>",pr:prid},function(index){
				num=index;	
...
<br>
<form action="<?php echo U('addru');?>" method="post" class="form-horizontal" id="formid" enctype="multipart/form-data" >
...
<input type="hidden" name="xf_contractid" id="xf_contractid" value="<?php echo ($info[id]); ?>">
  ...
  <div class="form-group">
    ...
    <div class="col-sm-3">
   	    <label class="radio-inline">
   	      <input name="type" type="radio" id="type_0" value="1" <?php echo ($info[type]=='1'?'checked':''); ?>  disabled>
   	      普通合同</label>
   	    
   	    <label class="radio-inline">
   	      <input type="radio" name="type" value="2" id="type_1" <?php echo ($info[type]=='2'?'checked':''); ?> disabled>
   	      框架合同</label>
   
    </div>
    
    <label for="inputEmail3" class="col-sm-1 control-label" <?php echo ($info[type]=='1'?'style="display:none;"':''); ?> >保证金</label>
    <div class="col-sm-3 bzj" <?php echo ($info[type]=='1'?'style="display:none;"':''); ?>>
...
<hr>
  ...
  <div class="form-group">
    ...
    <div class="col-sm-3">
    	<input id="contract_start" class="Wdate form-control" type="text" name="contract_start" onFocus="WdatePicker({maxDate:'#F{$dp.$D(\'contract_start\')||\'2020-10-01\'}'})" value="<?php echo (date('Y-m-d',$info[contract_start])); ?>"/> 
    </div>
    <label for="inputEmail3" class="col-sm-1 control-label">结束时间</label>
        <div class="col-sm-2">
<input id="contract_end" class="Wdate form-control" type="text" name="contract_end"  onFocus="WdatePicker({minDate:'#F{$dp.$D(\'contract_end\')}',maxDate:'2020-10-01'})" value="<?php echo (date('Y-m-d',$info[contract_end]?$info[contract_end]:'')); ?>"/>
    </div>
  ...
  <div class="form-group">
    ...
    <div class="col-sm-3">
    	<select  class="form-control" name="account" id="account" >
        	...
        	<?php if(is_array($account)): $i = 0; $__LIST__ = $account;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$account): $mod = ($i % 2 );++$i;?><option value="<?php echo ($account[id]); ?>"><?php echo ($account[a_users]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
        </select>
    </div>
