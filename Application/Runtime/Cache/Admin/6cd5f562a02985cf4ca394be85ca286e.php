...
<input id="contract_end" class="Wdate form-control" type="text" name="time_end"  placeholder="结束时间" value="<?php echo ($time_end); ?>"  onFocus="WdatePicker({minDate:'#F{$dp.$D(\'contract_start\')}',maxDate:'2020-10-01'})"/>
  ...
  <button type="submit" class="btn btn-primary">搜索</button>
  	<?php if(($ser_txt != '') OR ($time_end != '')): ?><a class="btn btn-info" href="<?php echo U("index2");?>">清除搜索条件</a><?php endif; ?>
...
<table class="table table-hover table-striped">
    ...
    <?php if(is_array($list)): $k = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$list): $mod = ($k % 2 );++$k;?><tr>
    	<td class="htshow" id="<?php echo U("show?id=$list[id]&yid2=$list[xf_hetonghao]");?>"><?php echo ($k); ?></td>
    	<td class="htshow" id="<?php echo U("show?id=$list[id]&yid2=$list[xf_hetonghao]");?>"><a href="<?php echo U("Customer/show?id=$list[aid]");?>"><?php echo ($list[advertiser]); ?></a></td>
    	<td class="htshow" id="<?php echo U("show?id=$list[id]&yid2=$list[xf_hetonghao]");?>"><?php echo ($list[appname]); ?></td>
    	<td class="htshow" id="<?php echo U("show?id=$list[id]&yid2=$list[xf_hetonghao]");?>" align="right"><?php echo (num_format($list[contract_money])); ?>&nbsp;&nbsp;&nbsp;&nbsp;</td>
        <td class="htshow" id="<?php echo U("show?id=$list[id]&yid2=$list[xf_hetonghao]");?>"><?php echo ($list[rebates_proportion]); ?>%</td>
    	<td class="htshow" id="<?php echo U("show?id=$list[id]&yid2=$list[xf_hetonghao]");?>"><?php echo ($list[a_users]); ?></td>
    	<!--<td>2016年8月15日</td>-->
    	<td class="htshow" id="<?php echo U("show?id=$list[id]&yid2=$list[xf_hetonghao]");?>"><?php echo ($list[name]); ?></td>
    	<td class="htshow" id="<?php echo U("show?id=$list[id]&yid2=$list[xf_hetonghao]");?>"><?php echo (date("y-m-d",$list[ctime])); ?></td>
       <!-- <td>
    	...
    	<td class="htshow" id="<?php echo U("show?id=$list[id]&yid2=$list[xf_hetonghao]");?>">
        ...
        <td class="htshow" id="<?php echo U("show?id=$list[id]&yid2=$list[xf_hetonghao]");?>"><?php echo ($list[submituser]); ?></td>
        <td>
        <!--<a href="#">发票</a>&nbsp;&nbsp;|&nbsp;&nbsp;<a href="#">财务</a><br>-->
        <a href="<?php echo U("updata?id=$list[id]&yid=$list[xf_contractid]");?>"  data-toggle="tooltip" data-placement="left" title="修改" ><span class="glyphicon glyphicon-pencil"></span></a>&nbsp;&nbsp;|&nbsp;&nbsp;<a href="<?php echo U("delete?id=$list[id]&yid=$info[id]");?>" <?php echo ($confirm); ?>  data-toggle="tooltip" data-placement="left"  title="删除"><span class="glyphicon glyphicon-trash"></span></a>
        </td>
    </tr><?php endforeach; endif; else: echo "" ;endif; ?>
</table>
<div class="row">
	<div class="col-sm-10"><?php echo ($page); ?></div>
	<div class="col-sm-2 text-right"><a href="<?php echo U("excel?searchtype=$type&search_text=$ser_txt&shenhe=$type2&pr_line=$type3&guidang=$type4&time_start=$time_start&time_end=$time_end&httype=$httype&yid=$info[id]");?>" style=" margin-top:20px;"  type="button" class="btn btn-primary  btn-sm"><span class="glyphicon glyphicon-save"></span> 导出Excel</a>
</div>
