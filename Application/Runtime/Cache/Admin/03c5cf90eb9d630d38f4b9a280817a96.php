...
<form class="form-inline" method="get">
  <div class="form-group">
     <select  class="form-control" name="searchtype">
     	...
     	<option value="submitname"  <?php echo ($type=='submitname'?'selected':''); ?>>客户所属</option>
     </select>
  </div>
  <div class="form-group">
  ...
  <select  class="form-control" name="guidang">
     	<option value="k"   <?php echo ($type4=='k'?'selected':''); ?> >是否归档</option>
     	<option value="1"  <?php echo ($type4=='1'?'selected':''); ?>>已归档</option>
     	<option value="0"  <?php echo ($type4=='0'?'selected':''); ?>>未归档</option>
  </select>
...
<table class="table table-hover table-striped">
    ...
    <?php if(is_array($list)): $k = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$list): $mod = ($k % 2 );++$k;?><tr>
    	...
    	<td class="htshow" id="<?php echo U("show?id=$list[id]");?>"><a href="<?php echo U("Customer/show?id=$list[aid]");?>"><?php echo ($list[advertiser]); ?></a> <?php if($list[isguidang] == 1): ?><span  data-toggle="tooltip" data-placement="left" title="合同已归档" class="glyphicon glyphicon-folder-close" style="color:#0492d9;"></span><?php endif; ?></td>
    	<td class="htshow" id="<?php echo U("show?id=$list[id]");?>"><?php echo ($list[appname]); ?></td>
    	<td class="htshow" id="<?php echo U("show?id=$list[id]");?>" align="right"><?php echo (num_format($list[contract_money])); ?>&nbsp;&nbsp;&nbsp;&nbsp;</td>
       ...
       <!-- <td>
        ...
        <td>
        <a href="<?php echo U("Renew/index?id=$list[id]");?>" data-toggle="tooltip" data-placement="left" title="续费"><span class="glyphicon glyphicon-transfer"></span></a>&nbsp;|
        <a  href="<?php echo U("Account/index?contract_id=$list[id]");?>"   data-toggle="tooltip" data-placement="left" title="账户"><span class="glyphicon glyphicon-list"></span></a>&nbsp;|
        <?php if($list[isguidang] == 0): endif; ?><!--归档限制 暂时取消-->
        <a href="<?php echo U("updata?id=$list[id]");?>"    data-toggle="tooltip" data-placement="left" title="修改" ><span class="glyphicon glyphicon-pencil"></span></a>&nbsp;|
...
<div class="row">
	...
	<div class="col-sm-2 text-right"><a href="<?php echo U("excel?searchtype=$type&search_text=$ser_txt&shenhe=$type2&pr_line=$type3&guidang=$type4&time_start=$time_start&time_end=$time_end&httype=$httype");?>" style=" margin-top:20px;"  type="button" class="btn btn-primary  btn-sm"><span class="glyphicon glyphicon-save"></span> 导出Excel</a>
</div>
</div>
</div>
